...
fun WifiBleTestScreen(
    ...
    // ANDROID INTEGRATION: Media server handler
    val handleOpenMediaServer: () -> Unit = {
        val url = viewModel.openMediaServer()
        if (url != null) {
            onNavigateToWebConsole()
        }
    }
        ...
        LazyColumn(
            ...
            item {
                WebConsoleSection(
                    ...
                    onOpenMediaServer = handleOpenMediaServer,
                )
            }
...
private fun WebConsoleSection(
    ...
    onOpenConsole: () -> Unit,
    onCloseConsole: () -> Unit,
    onOpenMediaServer: () -> Unit, // ANDROID INTEGRATION: Media server button
) {
    Card {
        Column(
            modifier =
            ...
            Row(
                ...
                // ANDROID INTEGRATION: Media server button
                Button(
                    modifier = Modifier.weight(1f),
                    onClick = onOpenMediaServer,
                    enabled = deviceIp.isNotBlank(),
                ) {
