...
class BleManager
    ...
    constructor(
            ...
            object : BluetoothGattCallback() {
                ...
                override fun onServicesDiscovered(
                    ...
                    if (status == BluetoothGatt.GATT_SUCCESS) {
                        ...
                        val profile =
                            BleConstants.SUPPORTED_SERVICE_PROFILES.firstOrNull { candidate ->
        ...
        fun startScan() {
                ...
                _connectionState.value =
                    BleConnectionState.Error(
                        ...
                        BleError.PermissionDenied(
                            Manifest.permission.BLUETOOTH_SCAN.takeIf {
            ...
            // Build scan filter
            val scanFilters =
                BleConstants.SCAN_SERVICE_UUIDS
                    .distinct()
                    .map { uuid ->
                        ScanFilter.Builder()
        ...
        suspend fun sendWifiConfig(
            ...
            return withContext(Dispatchers.IO) {
                try {
                    ...
                    val gatt =
                        bluetoothGatt ?: return@withContext Result.failure(
        ...
        suspend fun sendRawPayload(
            ...
            withContext(Dispatchers.IO) {
                try {
                    ...
                    val characteristic =
                        getWritableCharacteristic(gatt)
                            ?: return@withContext Result.failure(BleError.CharacteristicNotFound())
        ...
        private suspend fun writeCharacteristicPayload(
            ...
            val payloadSize =
                (negotiatedMtu - BleConstants.GATT_MTU_OVERHEAD)
        ...
        private fun recordTransportEvent(
            direction: BleTransportDirection,
            payload: ByteArray,
        ) {
                ...
                for (attempt in 1..BleConstants.MAX_RECONNECT_ATTEMPTS) {
                    _connectionState.value =
                        BleConnectionState.Reconnecting(
                            device,
                            attempt,
                            BleConstants.MAX_RECONNECT_ATTEMPTS,
                        )
